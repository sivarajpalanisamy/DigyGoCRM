...
function SentByBadge({ sentBy }: { sentBy?: string }) {
  if (!sentBy) return <span className="text-[#9ca3af]">-</span>;
  const map: Record<string, { label: string; cls: string }> = {
    automation: { label: 'Automation', cls: 'bg-purple-50 text-purple-600' },
    manual:     { label: 'Manual',     cls: 'bg-[#eef1f4] text-[#6b7280]'  },
  ...
  return (
    <span className={cn('text-[11px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap', cfg.cls)}>
      {cfg.label}
    </span>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          <h1 className="text-[18px] font-bold text-[#111318]">WhatsApp Personal - Overview</h1>
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-[14px] font-semibold text-[#6b7280] bg-white border border-[var(--hairline)] rounded-xl px-3 py-1.5 hover:bg-[var(--surface-2)] hover:text-[#111318] transition active:scale-[0.98]"
        >
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {PERIODS.map((pr) => (
          <button key={pr.value} onClick={() => setPeriod(pr.value)}
            className={cn(
              'text-[14px] font-semibold px-3.5 py-1.5 rounded-full border transition-all active:scale-[0.98]',
              period === pr.value
                ? 'bg-primary text-white border-primary'
                : 'bg-white text-[#6b7280] border-[var(--hairline)] hover:bg-[var(--surface-2)] hover:text-[#111318]',
            )}>
            {pr.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Volume chart */}
        <div className="lg:col-span-3 bg-white rounded-2xl border border-[var(--hairline)] p-5 card-shadow">
          <div className="mb-4">
          ...
          {loading ? (
            ...
            <div className="h-[210px] flex items-center justify-center text-[15px] text-[#8b929c]">
        ...
        {/* Top contacts */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[var(--hairline)] p-5 card-shadow">
          <div className="mb-4">
      ...
      <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow overflow-hidden">
        ...
        <div className="px-5 py-4 border-b border-[var(--hairline)] flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {/* Direction filter */}
            <div className="flex bg-[var(--surface-2)] p-1 rounded-full text-[12px] font-semibold">
              {(['all', 'sent', 'received'] as const).map((d) => (
                <button key={d} onClick={() => { setDirection(d); setLogPage(0); }}
                  className={cn(
                    ...
                    direction === d ? 'bg-primary text-white' : 'text-[#6b7280] hover:text-[#111318]',
                  )}>
                  {d}
                </button>
              ))}
            ...
            <div className="relative">
              ...
              <input
                ...
                className="pl-8 pr-3 py-1.5 text-[14px] border border-[var(--hairline)] rounded-xl w-48 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition"
              />
            </div>
          </div>
        </div>
                ...
                {logs.map((row) => (
                  <tr key={row.id} className="hover:bg-[var(--surface-2)] transition-colors">
                    ...
                    <td className="px-3 py-3 max-w-[240px]">
                      {row.body ? (
                        <p className="text-[#3a3f47] line-clamp-2 leading-relaxed">{row.body}</p>
                      ) : (
          ...
          <div className="px-5 py-3 border-t border-[var(--hairline)] flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button disabled={logPage === 0} onClick={() => setLogPage((p) => p - 1)}
                className="p-1.5 rounded-xl border border-[var(--hairline)] hover:bg-[var(--surface-2)] disabled:opacity-40 disabled:cursor-not-allowed transition active:scale-[0.98]">
