      ...
      <button
        ...
        className={cn(
          ...
          added ? 'border-emerald-200 bg-emerald-50 cursor-not-allowed'
                : 'border-[var(--hairline)] bg-white hover:border-primary/30 hover:bg-primary/5',
        )}
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={cn('text-[15px] font-semibold truncate', added ? 'text-emerald-700' : 'text-[#111318]')}>{label}</p>
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[80vh]">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--hairline)]">
          <div>
            ...
            <p className="text-[12px] text-[#6b7280] mt-0.5">Pick a CRM field to add to the booking form</p>
          </div>
...
export default function CalendarEditPage() {
  ...
  const inp = 'w-full border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 bg-white transition-colors';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/calendar')}
            className="p-2 rounded-xl hover:bg-[var(--accent-tint)] text-[#6b7280] hover:text-[#111318] transition-colors">
            <ArrowLeft className="w-5 h-5" />
          </button>
          <h2 className="font-headline text-[18px] font-bold text-[#111318]">
            {isEdit ? form.name || 'Edit Calendar' : 'New Calendar'}
          </h2>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/calendar')}
            className="px-4 py-2.5 rounded-xl text-[15px] font-semibold text-[#6b7280] border border-[var(--hairline)] bg-white hover:bg-[var(--accent-tint)] transition-colors">
            Cancel
          </button>
          <button onClick={handleSave} disabled={saving}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-[15px] font-bold text-white transition-all hover:-translate-y-0.5 disabled:opacity-60"
            style={shadowStyle}>
      ...
      <div className="flex border-b border-[var(--hairline)]">
        {(['details', 'availability', 'fields'] as const).map((s) => (
          <button key={s} onClick={() => setSection(s)}
            className={cn('px-5 py-3 text-[15px] font-semibold border-b-2 transition-colors',
              section === s ? 'border-primary text-primary' : 'border-transparent text-[#6b7280] hover:text-[#111318]')}>
      ...
      {/* ── DETAILS ── */}
      {section === 'details' && (
        <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-8 space-y-6" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-10">
            <div className="space-y-5">
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Calendar name <span className="text-red-500">*</span></label>
                <input className={inp} placeholder="HawcusSlotBooking" value={form.name} onChange={(e) => upd('name', e.target.value)} autoFocus />
              </div>
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-2 block">Assign Staff</label>
                <div className="border border-[var(--hairline)] rounded-xl overflow-hidden divide-y divide-[var(--hairline)]">
                  {activeStaff.length === 0 ? (
                    <p className="px-4 py-3 text-[15px] text-[#9ca3af]">No active staff found. Add staff in the Staff section.</p>
                    ...
                    <label key={member.id} className="flex items-center gap-3 px-4 py-3 hover:bg-[var(--app-bg)] cursor-pointer">
                      ...
                      />
                      <span className="text-[15px] text-[#111318]">{member.name}</span>
                      <span className="text-[12px] text-[#9ca3af] ml-1">{member.email}</span>
                    </label>
                  ))}
                </div>
              </div>
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Description</label>
            ...
            <div className="space-y-5">
              {/* Staff type cards */}
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-2 block">Staff Type</label>
                <div className="grid grid-cols-2 gap-2">
                  {([
                    ...
                    <button key={key} type="button" onClick={() => upd('staffType', key)}
                      ...
                      <p className={cn('text-[15px] font-bold', form.staffType === key ? 'text-primary' : 'text-[#111318]')}>{title}</p>
                ...
                {form.staffType === 'multi' && (
                  <div className="mt-3">
                    <label className="text-[14px] font-semibold text-[#6b7280] mb-1.5 block uppercase tracking-wide">Assignment Mode</label>
                    <div className="grid grid-cols-2 gap-2">
                      {([
                        ...
                        <button key={key} type="button" onClick={() => upd('assignmentMode', key)}
                          className={cn('text-left px-3 py-2.5 rounded-xl border-2 transition-all',
                            form.assignmentMode === key
                              ...
                              : 'border-gray-200 bg-white hover:border-gray-300')}>
                          <p className={cn('text-[14px] font-bold', form.assignmentMode === key ? 'text-blue-600' : 'text-[#111318]')}>{title}</p>
              ...
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-2 block">Location <span className="text-red-500">*</span></label>

                {/* Selected location pill */}
                {form.meetingType && (
                  <div className="flex items-center gap-2.5 border border-[var(--hairline)] rounded-xl px-3 py-2.5 mb-2 bg-white">
                    <div className="w-5 h-5 rounded shrink-0 flex items-center justify-center text-[11px] font-bold" style={{ background: 'linear-gradient(135deg,#4285F4 0%,#0F9D58 50%,#EA4335 100%)' }}>
                      <span className="text-white text-[9px] font-extrabold">G</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] text-[#111318] font-medium">{form.meetingType}</p>
                      {form.meetingLink
                        ? <p className="text-[12px] text-blue-500 truncate">{form.meetingLink}</p>
                        : <p className="text-[12px] text-[#9ca3af]">No link added</p>
                      }
                    </div>
                    <button onClick={() => openLocationModal(form.meetingType)}
                      className="p-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[#9ca3af] hover:text-primary transition-colors" title="Edit link">
                      <Pencil className="w-3.5 h-3.5" />
                    </button>
                    <button onClick={() => { upd('meetingType', ''); upd('meetingLink', undefined); }}
                      className="p-1.5 rounded-lg hover:bg-red-50 text-[#9ca3af] hover:text-red-400 transition-colors" title="Remove">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                )}
              ...
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Duration (min)</label>
              ...
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Max Bookings Per Day <span className="text-[#9ca3af] font-normal">(0 = unlimited)</span></label>
                <input type="number" min={0} className={inp} value={form.maxPerDay} onChange={(e) => upd('maxPerDay', Number(e.target.value))} />
              </div>
              <div>
                <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Capacity Per Slot <span className="text-[#9ca3af] font-normal">(1 = private · N = group · 0 = unlimited)</span></label>
                <input type="number" min={0} className={inp} value={form.capacityPerSlot} onChange={(e) => upd('capacityPerSlot', Number(e.target.value))} />
              </div>
            </div>
          </div>
          <div className="border-t border-[var(--hairline)] pt-6 grid grid-cols-1 sm:grid-cols-2 gap-8">
            <div>
              <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Minimum Scheduling Notice</label>
              <div className="flex items-center gap-2">
                <input type="number" className="w-24 border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none focus:border-primary/40" value={form.minNoticeValue} onChange={(e) => upd('minNoticeValue', Number(e.target.value))} />
                <div className="relative">
                  <select className="border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none appearance-none pr-8 bg-white" value={form.minNoticeUnit} onChange={(e) => upd('minNoticeUnit', e.target.value)}>
            ...
            <div>
              <label className="text-[15px] font-semibold text-[#111318] mb-1.5 block">Time Zone</label>
          ...
          {/* Scheduling window */}
          <div className="border-t border-[var(--hairline)] pt-6 space-y-3">
            <div>
              <label className="text-[15px] font-semibold text-[#111318] mb-1 block">Invitees can schedule…</label>
              <p className="text-[12px] text-[#6b7280] mb-3">How far into the future can someone book an appointment?</p>
                  ...
                  <button key={key} type="button" onClick={() => upd('schedulingType', key)}
                    className={cn('flex-1 min-w-[140px] text-left px-4 py-3 rounded-xl border-2 transition-all',
                      form.schedulingType === key
                        ? 'border-primary bg-orange-50'
                        : 'border-gray-200 bg-white hover:border-gray-300')}>
                    <p className={cn('text-[14px] font-bold', form.schedulingType === key ? 'text-primary' : 'text-[#111318]')}>{label}</p>
                    <p className="text-[12px] text-[#6b7280] mt-0.5">{desc}</p>
                  </button>
                ))}
              </div>
            ...
            {form.schedulingType === 'days' && (
              <div className="flex items-center gap-3 pl-1">
                <input type="number" min={1} max={365}
                  className="w-24 border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none focus:border-primary/40"
                  value={form.daysInFuture}
                  onChange={(e) => upd('daysInFuture', Number(e.target.value))} />
                <span className="text-[15px] text-[#6b7280]">calendar days into the future</span>
              </div>
            )}
            ...
            {form.schedulingType === 'range' && (
              <div className="flex items-center gap-3 pl-1 flex-wrap">
                <div>
                  <label className="text-[12px] font-bold text-[#6b7280] uppercase tracking-wide block mb-1">From</label>
                  <DatePicker
                    value={form.dateRangeStart ?? ''}
                    onChange={(v) => upd('dateRangeStart', v)}
                    placeholder="From date" />
                </div>
                <span className="text-[#9ca3af] mt-5">→</span>
                <div>
                  <label className="text-[12px] font-bold text-[#6b7280] uppercase tracking-wide block mb-1">To</label>
                  <DatePicker
                    value={form.dateRangeEnd ?? ''}
                    onChange={(v) => upd('dateRangeEnd', v)}
                    placeholder="To date" />
                </div>
              </div>
            )}
          ...
          {/* Redirect URL */}
          <div className="border-t border-[var(--hairline)] pt-6 space-y-1.5">
            <label className="text-[15px] font-semibold text-[#111318] block">Redirect URL <span className="text-[#9ca3af] font-normal text-[14px] normal-case">(optional)</span></label>
            <p className="text-[12px] text-[#6b7280]">After a successful booking, send the invitee to this URL (e.g. a thank-you page or payment link).</p>
            <input type="url"
              className={inp}
          ...
          <div className="flex items-center justify-between py-3 border-t border-[var(--hairline)]">
            <div>
              <p className="text-[15px] font-semibold text-[#111318]">Active</p>
              <p className="text-[12px] text-[#6b7280]">Allow invitees to book this calendar</p>
            </div>
      ...
      {section === 'availability' && (
        <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-4 sm:p-8" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_260px] gap-6 lg:gap-8">
            <div>
              <p className="text-[16px] font-bold text-[#111318] mb-5">Set your weekly hours</p>
                  ...
                  return (
                    <div key={day} className="py-3">
                      <div className="flex items-start gap-3">
                        <div className="flex items-center gap-2 w-16 sm:w-20 shrink-0 pt-0.5">
                          <input type="checkbox" checked={ds.enabled} onChange={() => toggleDay(day)} className="w-4 h-4 accent-primary cursor-pointer" />
                          <span className={cn('text-[15px] font-bold', ds.enabled ? 'text-[#111318]' : 'text-[#9ca3af]')}>{day}</span>
                        </div>
                        <div className="flex-1 min-w-0">
                          {!ds.enabled ? (
                            <span className="text-[15px] text-[#9ca3af]">Unavailable</span>
                          ) : (
                            <div className="space-y-1.5">
                              {ds.slots.map((slot, si) => (
                                <div key={si} className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
                                  <input type="time" value={slot.start} onChange={(e) => updateSlot(day, si, 'start', e.target.value)}
                                    className="border border-[var(--hairline)] rounded-lg px-2 sm:px-2.5 py-1.5 text-[14px] outline-none focus:border-primary/40 w-[104px] sm:w-28" />
                                  <span className="text-[14px] text-[#9ca3af]">-</span>
                                  <input type="time" value={slot.end} onChange={(e) => updateSlot(day, si, 'end', e.target.value)}
                                    className="border border-[var(--hairline)] rounded-lg px-2 sm:px-2.5 py-1.5 text-[14px] outline-none focus:border-primary/40 w-[104px] sm:w-28" />
                                  <button onClick={() => removeSlot(day, si)} className="p-1 rounded hover:bg-red-50 text-[#c3c8cf] hover:text-red-400 transition-colors ml-1">
                        ...
                        {ds.enabled && (
                          <div className="flex items-center gap-1 shrink-0 pt-0.5">
                            <button onClick={() => addSlot(day)} className="p-1.5 rounded hover:bg-[var(--accent-tint)] text-[#9ca3af] hover:text-primary transition-colors" title="Add slot">
                            ...
                            }} className="p-1.5 rounded hover:bg-[var(--accent-tint)] text-[#9ca3af] hover:text-primary transition-colors" title="Copy to all days">
            ...
            <div className="border-t lg:border-t-0 lg:border-l border-[var(--hairline)] pt-6 lg:pt-8 lg:pl-8">
              <p className="text-[15px] font-bold text-[#111318] mb-1">Date overrides</p>
              <p className="text-[14px] text-[#6b7280] mb-4 leading-relaxed">Set specific dates where your availability differs from the weekly schedule.</p>

              {Object.keys(form.dateOverrides ?? {}).length > 0 && (
                <div className="space-y-1.5 mb-3">
                  {Object.entries(form.dateOverrides ?? {}).sort().map(([date, ds]) => (
                    <div key={date} className="flex items-center justify-between px-3 py-2 bg-[var(--surface-2)] rounded-xl border border-[var(--hairline)]">
                      <span className="text-[14px] font-semibold text-[#111318]">{format(parseISO(date), 'MMM d, yyyy')}</span>
                      <span className="text-[12px] text-[#6b7280]">{ds.enabled ? `${ds.slots.length} slot(s)` : 'Off'}</span>
                      <button onClick={() => {
                        const updated = { ...(form.dateOverrides ?? {}) };
                        delete updated[date];
                        upd('dateOverrides', updated);
                      }} className="p-1 hover:bg-red-50 rounded text-[#c3c8cf] hover:text-red-400 transition-colors">
                        <X className="w-3 h-3" />
                      </button>
                    </div>
                  ))}
              ...
              <button
                ...
                className="w-full border border-[var(--hairline)] rounded-xl py-2.5 text-[15px] text-[#6b7280] hover:bg-[var(--app-bg)] transition-colors flex items-center justify-center gap-1.5"
              >
      ...
      {/* ── FORM FIELDS ── */}
      {section === 'fields' && (
        <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
          {/* Header */}
          <div className="px-6 py-4 border-b border-[var(--hairline)] flex items-center justify-between">
            <div>
              <p className="text-[16px] font-bold text-[#111318]">Booking Form Fields</p>
            ...
            {form.formFields.map((f) => {
              ...
              return (
                <div key={f.id} className="flex items-center gap-3 px-5 py-3.5 hover:bg-[var(--app-bg)] transition-colors group">
                  ...
                  <GripVertical className="w-4 h-4 text-[#c3c8cf] shrink-0 cursor-grab" />
                  ...
                  <div className="flex-1 min-w-0">
                    <input
                      ...
                      className="text-[15px] font-semibold text-[#111318] bg-transparent border-0 outline-none w-full focus:bg-white focus:border focus:border-primary/30 focus:rounded-lg focus:px-2 focus:py-0.5 transition-all"
                    />
                    {f.mapTo && (
                      <span className="text-[11px] text-emerald-600 font-medium bg-emerald-50 px-1.5 py-0.5 rounded-md">
                        → {f.mapTo}
                      </span>
                    )}
                  </div>
                  ...
                  {f.enabled && (
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      <span className="text-[12px] text-[#6b7280] w-14">Required</span>
                    </div>
                  )}
                  ...
                  {!isDefault ? (
                    <button
                      ...
                      className="p-1.5 rounded-lg hover:bg-red-50 text-[#c3c8cf] hover:text-red-500 transition-colors opacity-0 group-hover:opacity-100"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>
                  ) : (
          ...
          {form.formFields.length === 0 && (
            <div className="px-6 py-10 text-center text-[15px] text-[#9ca3af]">
      ...
      {showLocationModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--hairline)] flex items-center justify-between">
              <div>
                <h3 className="font-headline font-semibold text-[16px] text-[#111318]">{pendingLocationType}</h3>
                <p className="text-[14px] text-[#6b7280] mt-0.5">Paste your meeting link (optional)</p>
              </div>
              <button onClick={() => setShowLocationModal(false)} className="p-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[#6b7280]">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="px-6 py-5 space-y-3">
              <input
                ...
                autoFocus
                className="w-full border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none focus:border-primary/40 placeholder:text-[#c3c8cf]"
              />
              {locationLinkDraft.trim() && (
                <a href={locationLinkDraft.trim()} target="_blank" rel="noopener noreferrer"
                  className="inline-flex items-center gap-1.5 text-[14px] text-blue-500 hover:underline">
            ...
            <div className="px-6 pb-5 flex gap-2">
              <button onClick={() => setShowLocationModal(false)}
                className="flex-1 py-2.5 rounded-xl text-[15px] font-bold text-[#6b7280] border border-[var(--hairline)] bg-white hover:bg-[var(--app-bg)]">
                Cancel
              </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[var(--hairline)]">
              <h3 className="font-headline font-semibold text-[16px] text-[#111318]">Date Override</h3>
              <p className="text-[14px] text-[#6b7280] mt-0.5">Set availability for a specific date</p>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div>
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1.5 block">Date</label>
                <DatePicker value={overrideDate} onChange={setOverrideDate}
                  min={format(new Date(), 'yyyy-MM-dd')}
                  placeholder="Select date" className="w-full" />
              </div>
              <div className="flex items-center justify-between">
                <span className="text-[15px] font-semibold text-[#111318]">Available on this date</span>
              ...
              {overrideDay.enabled && (
                <div className="space-y-2">
                    ...
                    <div key={i} className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
                      ...
                      {overrideDay.slots.length > 1 && (
                        <button onClick={() => setOverrideDay((p) => ({ ...p, slots: p.slots.filter((_, si) => si !== i) }))}
                          className="p-1 rounded hover:bg-red-50 text-[#c3c8cf] hover:text-red-400 transition-colors">
                          <Trash2 className="w-3.5 h-3.5" />
                        </button>
                      )}
                    </div>
                  ...
                  <button onClick={() => setOverrideDay((p) => ({ ...p, slots: [...p.slots, { start: '09:00', end: '09:30' }] }))}
                    className="text-[14px] text-[#6b7280] flex items-center gap-1 hover:text-primary transition-colors mt-1">
