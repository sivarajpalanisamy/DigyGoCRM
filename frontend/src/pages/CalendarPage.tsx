...
function ApptPill({ event, onClick }: { event: CalendarEvent; onClick: () => void }) {
  ...
  return (
    <button type="button" onClick={(e) => { e.stopPropagation(); onClick(); }}
      ...
      <span className={cn('flex-1 px-2 py-[3.5px] text-[12px] font-medium truncate', AS[st].bg, AS[st].text)}>
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-none">
        <div className="pointer-events-auto w-full max-w-[380px] mx-4 bg-white rounded-2xl border border-[var(--hairline)] overflow-hidden"
          ...
          <div className="px-5 pt-4 pb-3">
            <div className="flex items-start justify-between gap-2 mb-4">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  <p className="text-[16px] font-bold text-[#111318] leading-snug truncate">{event.leadName || 'Unknown Lead'}</p>
                  <p className="text-[12px] text-[#6b7280]">{event.title}</p>
                </div>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => setEditing((v) => !v)}
                  className={cn('p-1.5 rounded-lg transition-colors', editing ? 'bg-orange-100 text-[var(--brand-dark)]' : 'hover:bg-[var(--accent-tint)] text-[#6b7280]')}>
                  <Pencil className="w-3.5 h-3.5" />
                </button>
                <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[#6b7280] transition-colors">
                  <X className="w-4 h-4" />
                </button>
              </div>
            </div>
            ...
            {!editing ? (
              <div className="space-y-2.5 pl-1">
                <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[12px] font-bold border', sData.bg, sData.text, sData.border)}>
                ...
                <div className="flex items-center gap-2 text-[14px] text-[#6b7280]">
                  <CalendarDays className="w-3.5 h-3.5 shrink-0 text-[#6b7280]" />
                  <span>{format(parseISO(event.date), 'EEEE, MMM d')} · {fmt12(event.time)} · {event.duration} min · <span className="text-[12px]">IST</span></span>
                </div>
                {event.email && (
                  <div className="flex items-center gap-2 text-[14px] text-[#6b7280]">
                ...
                {event.meetingLink && (
                  <a href={event.meetingLink} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-white transition-all hover:opacity-90"
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Date</label>
                    <DatePicker value={editForm.date} onChange={(v) => ef('date', v)} className="w-full" />
                  </div>
                  <div>
                    <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Time</label>
                    ...
                    <input type="time" value={editForm.time} onChange={(e) => ef('time', e.target.value)}
                      className="w-full border border-[var(--hairline)] rounded-xl px-3 py-2 text-[14px] outline-none focus:border-primary/40" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Type</label>
                    <select value={editForm.type} onChange={(e) => ef('type', e.target.value)}
                      className="w-full border border-[var(--hairline)] rounded-xl px-3 py-2 text-[14px] outline-none focus:border-primary/40 bg-white">
                  ...
                  <div>
                    <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Duration</label>
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Staff</label>
                ...
                <div>
                  <label className="text-[11px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Meeting Link</label>
                  <input value={editForm.meetingLink} onChange={(e) => ef('meetingLink', e.target.value)}
                    placeholder="https://meet.google.com/…"
                    className="w-full border border-[var(--hairline)] rounded-xl px-3 py-2 text-[14px] outline-none focus:border-primary/40 placeholder:text-[#c3c8cf]" />
                </div>
                <div className="flex gap-2 pt-1">
                  <button onClick={() => { setEditing(false); setIsRescheduling(false); }}
                    className="flex-1 py-2 rounded-xl text-[14px] font-bold text-[#6b7280] border border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)]">Cancel</button>
                  <button onClick={handleSaveEdit} disabled={saving}
                    className="flex-1 py-2 rounded-xl text-[14px] font-bold text-white disabled:opacity-50" style={shadowStyle}>
          ...
          {!editing && (st === 'scheduled' || st === 'rescheduled') && (
            <div className="px-5 py-3 border-t border-[#f1f3f5] flex flex-wrap gap-2">
              <button onClick={() => setPendingAction({ type: 'status', status: 'completed', label: 'Show Up', msg: 'Mark this appointment as shown up?' })}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-green-600 bg-green-50 border border-green-200 hover:bg-green-100 transition-colors">
              ...
              <button onClick={() => setPendingAction({ type: 'status', status: 'no-show', label: 'No Show', msg: 'Mark this appointment as no-show?' })}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-amber-600 bg-amber-50 border border-amber-200 hover:bg-amber-100 transition-colors">
                <AlertTriangle className="w-3 h-3" /> No Show
              </button>
              {st === 'scheduled' && (
                <button onClick={() => { setIsRescheduling(true); setEditing(true); }}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-purple-600 bg-purple-50 border border-purple-200 hover:bg-purple-100 transition-colors">
              ...
              <button onClick={() => setPendingAction({ type: 'status', status: 'cancelled', label: 'Cancel', msg: 'Are you sure you want to cancel this appointment?' })}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-red-600 bg-red-50 border border-red-200 hover:bg-red-100 transition-colors">
                <Ban className="w-3 h-3" /> Cancel
              </button>
            </div>
          )}
          {!editing && (
            <div className="px-5 py-2.5 border-t border-[#f1f3f5] flex justify-end">
              <button onClick={() => setPendingAction({ type: 'delete' })}
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[12px] font-bold text-red-500 hover:bg-red-50 transition-colors">
            ...
            <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(2px)' }}>
              <div className="bg-white rounded-2xl w-full max-w-xs shadow-2xl overflow-hidden">
                <div className="px-6 pt-6 pb-4 text-center">
                  ...
                  <h3 className="text-[16px] font-bold text-[#111318] mb-1">
                ...
                <div className="px-6 pb-5 flex gap-2">
                  <button onClick={() => setPendingAction(null)}
                    className="flex-1 py-2.5 rounded-xl text-[15px] font-bold text-[#6b7280] border border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)] transition-colors">
                    Go Back
                  </button>
                  <button
                    onClick={() => {
  ...
  return (
    <div className="px-3 py-3">
      <div className="flex items-center justify-between mb-2 px-1">
        <button onClick={() => setMiniMonth((m) => subMonths(m, 1))} className="p-1 rounded-lg hover:bg-[var(--accent-tint)] text-[#6b7280] transition-colors"><ChevronLeft className="w-3 h-3" /></button>
        <p className="text-[12px] font-bold text-[#111318]">{format(miniMonth, 'MMM yyyy')}</p>
        <button onClick={() => setMiniMonth((m) => addMonths(m, 1))} className="p-1 rounded-lg hover:bg-[var(--accent-tint)] text-[#6b7280] transition-colors"><ChevronRight className="w-3 h-3" /></button>
      </div>
      <div className="grid grid-cols-7 mb-1">
        {['S','M','T','W','T','F','S'].map((d, i) => (
          <div key={i} className="text-center text-[10px] font-bold text-[#9ca3af] py-0.5">{d}</div>
        ))}
      </div>
        ...
        {days.map((day) => {
          ...
          return (
            <button key={day.toISOString()} onClick={() => onDayClick(day)}
              className={cn('w-7 h-7 mx-auto flex items-center justify-center rounded-full text-[11.5px] font-medium transition-all',
                ...
                inM         ? 'text-[#111318] hover:bg-[var(--accent-tint)]' :
                              'text-[#c3c8cf]',
              )}>
              {format(day, 'd')}
            </button>
...
function CalWeekView({ cursor, byDay, visibleStatuses, onEventClick, onDayClick }: {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="grid grid-cols-7 border-b border-[#eef1f4] bg-white shrink-0">
        {days.map((day) => (
          <div key={day.toISOString()} className="border-r border-[#eef1f4] last:border-r-0 py-3 flex flex-col items-center gap-1">
            <p className="text-[11px] font-bold text-[#6b7280] uppercase tracking-widest">{format(day, 'EEE')}</p>
            <button onClick={() => onDayClick(day)} className={cn('w-9 h-9 rounded-full flex items-center justify-center text-[19px] font-extrabold transition-all',
              isToday(day) ? 'bg-[var(--brand-dark)] text-white shadow-md' : 'text-[#111318] hover:bg-[var(--accent-tint)]')}>
              {format(day, 'd')}
            </button>
          </div>
        ))}
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid grid-cols-7 min-h-full">
          {days.map((day) => {
            ...
            return (
              <div key={key} className={cn('border-r border-[#eef1f4] last:border-r-0 p-1.5 space-y-[3px] min-h-[460px]',
                ...
                {more > 0 && (
                  <button type="button" onClick={() => onDayClick(day)}
                    className="w-full text-left text-[11px] font-semibold text-[#6b7280] pl-2 py-[2px] hover:text-[var(--brand-dark)] transition-colors">
                    +{more} more
                  </button>
                )}
              </div>
...
function CalMonthView({ cursor, byDay, visibleStatuses, onEventClick, onDayClick }: {
  ...
  return (
    <div className="flex flex-col flex-1 overflow-hidden">
      <div className="grid grid-cols-7 border-b border-[#eef1f4] shrink-0">
        {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d) => (
          <div key={d} className="text-center py-2.5 text-[11px] font-bold text-[#6b7280] uppercase tracking-widest border-r border-[#eef1f4] last:border-r-0">{d}</div>
        ))}
      </div>
      <div className="flex-1 overflow-y-auto">
          ...
          {days.map((day) => {
            ...
            return (
              <div key={day.toISOString()} onClick={() => onDayClick(day)}
                className={cn('border-r border-b border-[#eef1f4] last:border-r-0 min-h-[110px] p-1.5 cursor-pointer',
                  isToday(day) ? 'bg-orange-50/30' : !inMonth ? 'bg-[#fcfcfd]' : 'hover:bg-[var(--app-bg)]')}>
                <div className="flex justify-start mb-1">
                  <span className={cn('w-7 h-7 rounded-full flex items-center justify-center text-[14px] font-semibold',
                    isToday(day) ? 'bg-[var(--brand-dark)] text-white font-extrabold' :
                    inMonth ? 'text-[#111318]' : 'text-[#c3c8cf]')}>
                    {format(day, 'd')}
                  </span>
                </div>
                <div className="space-y-[3px]">
                  {visible.map((e) => <ApptPill key={e.id} event={e} onClick={() => onEventClick(e)} />)}
                  {more > 0 && <p className="text-[11px] text-[#6b7280] pl-1 font-medium">+{more} more</p>}
                </div>
              </div>
            );
          })}
...
function CalDayView({ cursor, byDay, visibleStatuses, onEventClick }: {
  ...
  return (
    <div className="flex-1 overflow-y-auto px-6 py-5">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl border border-[#eceef1] overflow-hidden">
        <div className={cn('px-6 py-5 border-b border-[#eef1f4]', isToday(cursor) && 'bg-orange-50/40')}>
          <p className="text-[11px] font-bold text-[#6b7280] uppercase tracking-widest">{format(cursor, 'EEEE')}</p>
          <div className="flex items-end justify-between mt-1">
            <p className={cn('text-[38px] font-extrabold font-headline leading-none', isToday(cursor) ? 'text-[var(--brand-dark)]' : 'text-[#111318]')}>{format(cursor, 'd')}</p>
            <p className="text-[15px] text-[#6b7280] mb-1">{format(cursor, 'MMMM yyyy')}</p>
          </div>
        </div>
        {items.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 gap-3">
            ...
            <p className="text-[16px] font-bold text-[#111318]">Nothing scheduled</p>
            <p className="text-[14px] text-[#6b7280]">No appointments for this day</p>
          </div>
        ) : (
              ...
              return (
                <button key={e.id} type="button" onClick={() => onEventClick(e)}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[15px] font-semibold text-[#111318]">{e.leadName || 'Unknown'}</p>
                    <div className="flex items-center gap-2 mt-0.5">
                      <span className="text-[12px] text-[#6b7280] flex items-center gap-1"><Clock className="w-3 h-3" />{fmt12(e.time)} · {e.duration} min</span>
                    </div>
                  </div>
                  <span className={cn('text-[12px] font-semibold px-2.5 py-1 rounded-full border shrink-0', AS[st].bg, AS[st].text, AS[st].border)}>{AS[st].label}</span>
                </button>
              );
            })}
          </div>
    ...
    <div className="space-y-4">
        ...
        <>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {([
              ...
              <div key={s.label} className="bg-white rounded-2xl px-4 py-3.5 border border-[var(--hairline)] flex items-center gap-3" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                ...
                <div>
                  ...
                  <p className="text-[20px] font-semibold text-[#111318] leading-tight">{s.value}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              <select value={calendarFilter} onChange={(e) => setCalendarFilter(e.target.value)}
                className="appearance-none pl-4 pr-10 py-2.5 bg-white border border-[var(--hairline)] rounded-xl text-[15px] font-semibold text-[#111318] outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition cursor-pointer min-w-[160px]"
            ...
            <div className="relative">
              <select value={staffFilter} onChange={(e) => setStaffFilter(e.target.value)}
                className={cn(
                  'appearance-none pl-4 pr-10 py-2.5 bg-white border border-[var(--hairline)] rounded-xl text-[15px] font-semibold outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 transition cursor-pointer min-w-[160px]',
                  staffFilter === 'all' ? 'text-[#9ca3af]' : 'text-[#111318]'
                )}
                style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            ...
            <button className="ml-auto flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-[14px] font-bold text-white"
              style={shadowStyle}
          ...
          <div className="flex flex-col lg:flex-row gap-5 items-stretch lg:items-start">
            {/* Left: month grid */}
            <div ref={calRef} className="flex-[2] bg-white rounded-2xl border border-[var(--hairline)] p-4" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-headline text-[17px] font-semibold text-[#111318]">{format(currentMonth, 'MMMM yyyy')}</h3>
                <div className="flex items-center gap-1">
                  <button onClick={() => setCurrentMonth(subMonths(currentMonth, 1))} className="w-8 h-8 rounded-lg hover:bg-[var(--accent-tint)] flex items-center justify-center text-[#6b7280]">
                    <ChevronLeft className="w-4 h-4" />
                  </button>
                  <button onClick={() => { setCurrentMonth(new Date()); setSelectedDate(new Date()); }} className="px-3 py-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[14px] font-semibold text-[#111318]">Today</button>
                  <button onClick={() => setCurrentMonth(addMonths(currentMonth, 1))} className="w-8 h-8 rounded-lg hover:bg-[var(--accent-tint)] flex items-center justify-center text-[#6b7280]">
              ...
              <div className="grid grid-cols-7 gap-1">
                {['Sun','Mon','Tue','Wed','Thu','Fri','Sat'].map((d) => (
                  <div key={d} className="text-center text-[11px] font-bold uppercase tracking-wider text-[#6b7280] pb-1.5">{d}</div>
                ))}
                {calDays.map((day, i) => {
                  ...
                  return (
                    <button key={day.toISOString()} onClick={() => setSelectedDate(day)}
                      className={cn('rounded-lg min-h-[50px] flex flex-col items-center justify-center gap-1 transition-colors',
                        ...
                        isTod ? 'bg-[#f1f3f5] text-[#111318]' :
                                'hover:bg-[var(--app-bg)] text-[#111318]',
                        !isSameMonth(day, currentMonth) && 'opacity-40'
                      )}>
                      <span className="font-semibold text-[15px] tabular-nums">{format(day, 'd')}</span>
                      {dayEvts.length > 0 && (
            ...
            <div className="flex-1 bg-white rounded-2xl border border-[var(--hairline)] flex flex-col"
              style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)', height: calHeight > 0 ? calHeight : undefined, maxHeight: calHeight > 0 ? calHeight : 600 }}>
              <div className="px-4 pt-4 pb-2.5 shrink-0 border-b border-[var(--hairline)]">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold text-[15px] text-[#111318]">
                    {selectedDate ? format(selectedDate, 'EEEE, MMM d') : 'Select a date'}
                  </h3>
                  {selectedEvents.length > 0 && (
                    <span className="text-[12px] font-medium text-[#6b7280] bg-black/[0.06] rounded-full px-2 py-0.5">{selectedEvents.length}</span>
                  )}
                </div>
              </div>
              {selectedEvents.length === 0 ? (
                <div className="text-center py-10 flex-1">
                  <Clock className="w-10 h-10 mx-auto mb-2 text-[#c3c8cf] opacity-40" />
                  <p className="text-[15px] text-[#6b7280]">No events for this date</p>
                </div>
              ) : (
                <div className="flex-1 overflow-y-auto px-3 pt-2 pb-1 space-y-2">
                  {selectedEvents.map((event) => {
                    ...
                    const st = getApptStatus(event);
                    return (
                      <div key={event.id} className="p-3 rounded-xl border border-[var(--hairline)] hover:shadow-sm transition-shadow cursor-pointer"
                        ...
                        <div className="flex items-start gap-2.5">
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-1">
                              <p className="font-semibold text-[14px] text-[#111318] leading-tight">{event.title}</p>
                              <span className={cn('text-[10px] border rounded-full font-medium shrink-0 ml-1 px-2 py-0.5', AS[st].bg, AS[st].text, AS[st].border)}>
                                {AS[st].label}
                              </span>
                            </div>
                            ...
                            <p className="text-[11px] text-[#6b7280]">{event.leadName}</p>
                            {event.meetingLink && (
                              <a href={event.meetingLink} target="_blank" rel="noreferrer"
                                onClick={(e) => e.stopPropagation()}
                                className="text-[11px] text-primary flex items-center gap-0.5 hover:underline font-semibold mt-1">
      ...
      {tab === 'create-edit' && (
        <div className="space-y-4">
          ...
          {etLoading ? (
            <div className="py-20 text-center text-[15px] text-[#9ca3af]">Loading…</div>
          ) : eventTypes.length === 0 ? (
            <div className="bg-white rounded-2xl border border-[var(--hairline)] py-20 flex flex-col items-center gap-3" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              <CalendarDays className="w-10 h-10 text-[#c3c8cf]" />
              <p className="text-[16px] font-semibold text-[#111318]">No calendars yet</p>
              <p className="text-[14px] text-[#6b7280]">Create a calendar - a shareable booking link is auto-generated</p>
              <button onClick={() => navigate('/calendar/edit/new')} className="mt-2 px-5 py-2.5 rounded-xl text-[15px] font-bold text-white" style={shadowStyle}><Plus className="w-4 h-4 inline mr-1" /> New Calendar</button>
            </div>
          ) : (
              ...
              {eventTypes.map((et) => {
                ...
                return (
                  <div key={et.id}
                    ...
                    className="bg-white rounded-2xl border border-[var(--hairline)] flex flex-col hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 cursor-pointer overflow-hidden"
                    style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                    <div className="h-[3px] w-full bg-gradient-to-r from-orange-400 to-orange-300" />
                    <div className="p-5 flex-1 space-y-3">
                      <div className="flex items-start justify-between">
                        <div className="flex-1 min-w-0">
                          <h3 className="font-headline font-bold text-[16px] text-[#111318] mb-0.5">{et.name}</h3>
                          <p className="text-[14px] text-[#6b7280]">{et.duration} min · {et.meetingType}</p>
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); navigate(`/calendar/edit/${et.id}`, { state: { eventType: et } }); }}
                          className="p-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[#9ca3af] hover:text-primary transition-colors" title="Edit">
                          <Settings2 className="w-4 h-4" />
                        </button>
                      </div>
                      {et.description && <p className="text-[14px] text-[#6b7280] line-clamp-2">{et.description}</p>}

                      {/* Auto-generated booking link */}
                      <div className="flex items-center gap-1.5 px-3 py-2 bg-[var(--app-bg)] rounded-xl border border-[var(--hairline)] cursor-pointer hover:bg-[#eef1f4] transition-colors" onClick={(e) => { e.stopPropagation(); window.open(bookingUrl, '_blank'); }}>
                        <ExternalLink className="w-3 h-3 text-[#9ca3af] shrink-0" />
                        <span className="text-[11px] text-[#6b7280] font-mono truncate flex-1">/book/{et.slug}</span>
                        <button onClick={(e) => { e.stopPropagation(); copyToClipboard(bookingUrl); toast.success('Link copied!'); }}
                          className="p-1 rounded hover:bg-[#eef1f4] text-[#9ca3af] hover:text-primary transition-colors shrink-0">
                          <Copy className="w-3 h-3" />
                        </button>
                      </div>
                    </div>
                    <div className="px-5 py-3 border-t border-[var(--hairline)] bg-[var(--app-bg)] flex items-center justify-between">
                      <div className="flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
                        ...
                        <span className={cn('text-[14px] font-semibold', et.isActive ? 'text-green-600' : 'text-[#9ca3af]')}>{et.isActive ? 'Active' : 'Inactive'}</span>
                      </div>
                      <button onClick={async (e) => {
                        e.stopPropagation();
                      ...
                      }} className="p-1.5 rounded-lg hover:bg-red-50 text-[#c3c8cf] hover:text-red-500 transition-colors">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {([
              ...
              <button key={f.key} onClick={() => setApptFilter(f.key)}
                className={cn('flex items-center gap-1.5 px-4 py-2 rounded-xl text-[14px] font-semibold border transition-all',
                  apptFilter === f.key ? 'text-white border-transparent' : 'text-[#6b7280] border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)]')}
                style={apptFilter === f.key ? shadowStyle : { boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
                {f.label}
                <span className={cn('text-[11px] rounded-full px-1.5 py-0.5 min-w-[20px] text-center font-bold',
                  apptFilter === f.key ? 'bg-white/20' : 'bg-black/[0.06] text-[#6b7280]')}>{f.count}</span>
              </button>
            ))}
          </div>

          {Object.keys(groupedApts).length === 0 ? (
            <div className="bg-white rounded-2xl border border-[var(--hairline)] py-20 text-center" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
              <Clock className="w-10 h-10 mx-auto text-[#c3c8cf] mb-3" />
              <p className="text-[16px] font-semibold text-[#111318]">No appointments</p>
              <p className="text-[15px] text-[#6b7280] mt-1">No {apptFilter === 'all' ? '' : apptFilter} appointments found.</p>
            </div>
          ) : (
            Object.entries(groupedApts).map(([date, apts]) => (
              <div key={date}>
                <p className="text-[14px] font-bold uppercase tracking-wider text-[#6b7280] mb-2 px-1">{format(parseISO(date), 'EEEE, MMMM d, yyyy')}</p>
                <div className="space-y-3">
                  {apts.map((apt) => (
                    <div key={apt.id} className="bg-white rounded-2xl border border-[var(--hairline)] overflow-hidden" style={{ boxShadow: '0 1px 3px rgba(0,0,0,0.05)' }}>
                      <div className="px-6 py-4 flex items-center gap-5 cursor-pointer hover:bg-[var(--app-bg)] transition-colors"
                        ...
                        <div className="flex-1 min-w-0 flex items-center gap-6 flex-wrap">
                          <div className="min-w-[180px]">
                            <p className="text-[16px] font-bold text-[#111318]">{apt.leadName}</p>
                            <p className="text-[14px] text-[#6b7280]">{apt.email}</p>
                          </div>
                          ...
                          <span className="text-[14px] text-primary font-semibold">{apt.eventTypeName}</span>
                          <span className="text-[12px] text-[#9ca3af]">{apt.timezone}</span>
                        </div>
                        <span className={cn('text-[12px] font-semibold px-2.5 py-1 rounded-full border shrink-0', STATUS_BADGE[apt.status])}>{STATUS_LBL[apt.status]}</span>
                      </div>
                      <div className="px-6 py-3 border-t border-[var(--hairline)] bg-[var(--app-bg)] flex items-center gap-2 flex-wrap">
                        {(apt.status === 'booked' || apt.status === 'rescheduled') && [
                          ...
                          }} className={cn('flex items-center gap-1.5 px-4 py-2 rounded-xl text-[14px] font-bold border transition-colors', cls)}>
                            <Icon className="w-3.5 h-3.5" /> {label}
                          </button>
                        ))}
                        {apt.status === 'booked' && (
                          <button onClick={() => { const e = calendarEvents.find((ce) => ce.id === apt.id); if (e) setSelectedEvent(e); }}
                            className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-[14px] font-bold border transition-colors text-purple-600 bg-purple-50 border-purple-200 hover:bg-purple-100">
                        ...
                        }} className="ml-auto flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[14px] font-bold text-red-500 hover:bg-red-50 border border-transparent transition-colors">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(2px)' }}
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="px-6 py-4 border-b border-[var(--hairline)] flex items-center justify-between">
              <h3 className="font-headline font-semibold text-[16px] text-[#111318]">New Event</h3>
              <button onClick={() => { setShowNewEvent(false); setLeadSearch(''); setShowLeadDrop(false); }} className="p-1.5 rounded-lg hover:bg-[var(--accent-tint)] text-[#6b7280]"><X className="w-4 h-4" /></button>
            </div>
            <div className="px-6 py-5 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Date</label>
                  <DatePicker value={newEventForm.date} onChange={(v) => setNewEventForm((f) => ({ ...f, date: v }))} className="w-full" />
                </div>
                <div>
                  <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Time</label>
                  <select value={newEventForm.time} onChange={(e) => setNewEventForm((f) => ({ ...f, time: e.target.value }))}
                    className="w-full border border-[var(--hairline)] rounded-xl px-3 py-2.5 text-[15px] outline-none focus:border-primary/40 bg-white">
                    {TIME_OPTIONS.map((t) => <option key={t} value={t}>{fmt12(t)}</option>)}
                  </select>
                </div>
              </div>
              <div className="relative">
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Lead</label>
                <div className="relative">
                  <input
                    value={leadSearch}
                    ...
                    className="w-full px-3 py-2.5 rounded-xl border border-[var(--hairline)] text-[15px] outline-none focus:border-primary/40 placeholder:text-[#c3c8cf]" />
                ...
                {showLeadDrop && leadSearch.length > 0 && (() => {
                  ...
                  return (
                    <div className="absolute left-0 right-0 top-full mt-1 z-[60] bg-white border border-[var(--hairline)] rounded-xl shadow-lg overflow-hidden max-h-44 overflow-y-auto">
                      {matches.length > 0 ? matches.map((l) => (
                        <button key={l.id} type="button"
                          ...
                          <p className="text-[14px] font-semibold text-[#111318]">{l.firstName} {l.lastName}</p>
                          {l.email && <p className="text-[12px] text-[#6b7280]">{l.email}</p>}
                        </button>
                      )) : (
                        <p className="px-3 py-2 text-[14px] text-[#6b7280]">No match - will save as guest name</p>
                      )}
                    </div>
                  );
                })()}
              ...
              <div>
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Type</label>
                <div className="flex gap-2">
                  {(['call','demo','meeting'] as const).map((t) => (
                    <button key={t} onClick={() => setNewEventForm((f) => ({ ...f, type: t }))}
                      className={cn('flex-1 py-2 rounded-xl text-[14px] font-bold border capitalize transition-all',
                        newEventForm.type === t ? 'text-white border-transparent' : 'text-[#6b7280] border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)]')}
                      style={newEventForm.type === t ? shadowStyle : {}}>{t}</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Duration</label>
                <div className="flex gap-2">
                  {[15,30,60].map((d) => (
                    <button key={d} onClick={() => setNewEventForm((f) => ({ ...f, duration: d }))}
                      className={cn('flex-1 py-2 rounded-xl text-[14px] font-bold border transition-all',
                        newEventForm.duration === d ? 'text-white border-transparent' : 'text-[#6b7280] border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)]')}
                      style={newEventForm.duration === d ? shadowStyle : {}}>{d} min</button>
                  ))}
                </div>
              </div>
              <div>
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Assign Staff <span className="normal-case text-[#9ca3af] font-normal">(optional)</span></label>
              ...
              <div>
                <label className="text-[12px] font-bold uppercase tracking-wider text-[#4a4f57] mb-1 block">Meeting Link <span className="normal-case text-[#9ca3af] font-normal">(optional)</span></label>
            ...
            <div className="px-6 pb-5 flex gap-2">
              <button onClick={() => setShowNewEvent(false)} className="flex-1 py-2.5 rounded-xl text-[15px] font-bold text-[#6b7280] border border-[var(--hairline)] bg-white hover:bg-[var(--surface-2)]">Cancel</button>
              <button onClick={handleCreateEvent} disabled={newEventForm.leadName.trim().length < 2}
                className="flex-1 py-2.5 rounded-xl text-[15px] font-bold text-white disabled:opacity-40 transition-opacity" style={shadowStyle}>Create</button>
            </div>
          </div>
        </div>
      )}
