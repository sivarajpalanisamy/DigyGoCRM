...
export default function WaPersonalTemplateEditorPage() {
  ...
  return (
    ...
    <div className="flex flex-col flex-1 min-h-0 overflow-hidden -mx-3 -my-4 md:-mx-6 md:-my-5">
      ...
      {/* ── Header ── */}
      <header className="bg-white border-b border-[var(--hairline)] px-5 py-0 flex items-center justify-between shrink-0 h-14">
        ...
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          <span className="ml-1.5 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-50 border border-green-200 text-[12px] font-medium text-green-700 shrink-0">
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-y-auto md:overflow-hidden">
            ...
            <section>
              ...
              <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-5">
                ...
                <Input
                  ...
                  className="border-[var(--hairline)] focus:border-orange-300 focus:ring-1 focus:ring-orange-200 bg-[#ffffff]"
                />
              ...
              <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow overflow-hidden">
                ...
                <div className="flex items-center gap-1.5 px-4 py-2.5 border-b border-[var(--hairline)] bg-[#ffffff]">
                  ...
                  <FormatBtn onClick={() => wrapSelection('_')} title="Italic  _text_">
                    <span className="italic text-[15px]">I</span>
                  </FormatBtn>
                  ...
                  <span className="text-[12px] text-[#6b7280] hidden sm:block">Select text then click to format</span>
                  <span className="ml-auto text-[12px] text-[#6b7280]">{message.length} / 4096</span>
                </div>
                ...
                <div className="px-4 pb-4 pt-2 border-t border-[var(--hairline)]">
                  <button
            ...
            <section>
              ...
              <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-5 space-y-4">
                ...
                {existingFile && !removeFile && !file && (
                  <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-xl border border-[var(--hairline)]">
                    <FileTypeIcon type={existingFile.type} name={existingFile.name} className="w-5 h-5 text-orange-600 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] font-medium text-[#111318] truncate">{existingFile.name}</p>
                      <p className="text-[12px] text-[#6b7280]">Current attachment</p>
                    </div>
                    <button
                ...
                {file && (
                  <div className="flex items-center gap-3 p-3 bg-orange-50 rounded-xl border border-[var(--hairline)]">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-[15px] font-medium text-[#111318] truncate">{file.name}</p>
                      <p className="text-[12px] text-[#6b7280]">{(file.size / 1024).toFixed(0)} KB</p>
                    </div>
                    <button
                ...
                <div
                  ...
                  className={cn(
                    ...
                    dragOver
                      ...
                      : 'border-[var(--hairline)] bg-[#ffffff] hover:border-orange-300 hover:bg-orange-50/60',
                  )}
                >
                  <div className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center mx-auto mb-3">
                    <Upload className="w-5 h-5 text-orange-500" />
                  </div>
                  <p className="text-[15px] font-medium text-[#111318]">
        ...
        <div className="w-full md:w-[380px] shrink-0 border-t md:border-t-0 md:border-l border-[var(--hairline)] bg-[#fff7f0] md:overflow-y-auto">
          <div className="p-6 space-y-5">
                      ...
                      {showAttach && attachType === 'video' && (
                        <div className="flex items-center gap-2 bg-gray-100 px-3 py-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="text-[12px] font-medium text-gray-700 truncate max-w-[180px]">{attachName}</p>
                            <p className="text-[11px] text-gray-500">Video</p>
                          </div>
                        </div>
                      )}
                      {showAttach && attachType === 'doc' && (
                        <div className="flex items-center gap-2.5 border-b border-[var(--hairline)] bg-orange-50 px-3 py-2.5">
                          <div className="w-8 h-8 rounded-lg bg-orange-100 flex items-center justify-center shrink-0">
                            <FileText className="w-4 h-4 text-orange-600" />
                          </div>
                          <div className="min-w-0">
                            <p className="text-[12px] font-semibold text-gray-800 truncate max-w-[180px]">{attachName}</p>
                            <p className="text-[11px] text-gray-500">{attachName.split('.').pop()?.toUpperCase() || 'FILE'}</p>
                          </div>
                        </div>
                      )}
                          ...
                          <div className="mt-2 rounded-lg border border-gray-200 overflow-hidden bg-white">
                            ...
                            <div className="px-2.5 py-2 border-t border-gray-100 bg-[#f7f7f7]">
                              {linkPreview.siteName && (
                                <p className="text-[10px] text-gray-400 uppercase tracking-wide mb-0.5">{linkPreview.siteName}</p>
                              )}
                              {linkPreview.title && (
                                <p className="text-[12px] font-semibold text-gray-800 leading-tight line-clamp-2">{linkPreview.title}</p>
                              )}
                              {linkPreview.description && (
                                <p className="text-[11px] text-gray-500 mt-0.5 line-clamp-2 leading-snug">{linkPreview.description}</p>
                              )}
                            </div>
                          </div>
                        )}
            ...
            <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-4">
              <h3 className="text-[11px] font-bold text-[#92400e] uppercase tracking-widest mb-3">
                Preview Sample Values
              </h3>
              <div className="space-y-1.5">
                {allPickerFields.map((f) => (
                  <div key={f.slug} className="flex items-center justify-between text-[12px] gap-2">
                    <span className="font-mono shrink-0 text-orange-600">{`{%${f.slug}%}`}</span>
                    <span className="text-[#6b7280] text-right truncate">{f.preview || `[${f.name}]`}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-orange-50 rounded-2xl border border-[var(--hairline)] p-4">
              ...
              <div className="space-y-2 text-[14px] text-[#6b7280]">
                <div className="flex items-center gap-2">
                  <code className="bg-orange-100 text-orange-800 px-1.5 py-0.5 rounded text-[12px]">*text*</code>
                  <span>→</span>
                  <strong className="text-[#111318]">bold</strong>
                </div>
                <div className="flex items-center gap-2">
                  <code className="bg-orange-100 text-orange-800 px-1.5 py-0.5 rounded text-[12px]">_text_</code>
                  <span>→</span>
                  <em className="text-[#111318]">italic</em>
                </div>
                <div className="flex items-center gap-2">
                  <code className="bg-orange-100 text-orange-800 px-1.5 py-0.5 rounded text-[12px]">~text~</code>
                  <span>→</span>
                  <del className="text-[#111318]">strikethrough</del>
                </div>
                <p className="text-[12px] pt-1.5 border-t border-[var(--hairline)] text-[#6b7280]">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-[520px] max-w-full max-h-[80vh] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-[var(--hairline)] shrink-0">
              <div>
                ...
                <p className="text-[12px] text-[#6b7280] mt-0.5">Click any field - inserts <code className="bg-orange-50 px-1 rounded text-orange-700">{'{%slug%}'}</code> at cursor</p>
              </div>
              <button
            ...
            <div className="px-4 py-3 border-b border-[var(--hairline)] shrink-0">
              <div className="flex items-center gap-2 bg-[#ffffff] border border-[var(--hairline)] rounded-xl px-3 py-2">
                ...
                />
                {search && (
                  <button onClick={() => setSearch('')} className="text-[#6b7280] hover:text-[#111318]">
            ...
            {!search && (
              <div className="flex border-b border-[var(--hairline)] px-4 shrink-0 bg-white">
                ...
                searchResults.map((group) => (
                  <div key={group.label}>
                    ...
                    {group.items.map((item) => (
                      ...
                      >
                        <div className="min-w-0">
                          <p className="text-[15px] font-medium text-[#111318] truncate">{item.name}</p>
                          {item.preview && (
                            <p className="text-[12px] text-[#6b7280] mt-0.5 truncate">e.g. {item.preview}</p>
                          )}
                        </div>
