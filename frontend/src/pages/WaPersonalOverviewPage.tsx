...
function Trend({ value, prev }: { value: number; prev: number }) {
  ...
  if (prev === 0) return <span className="text-[11px] text-emerald-600 font-semibold">New</span>;
  const pct = Math.round(((value - prev) / prev) * 100);
  if (pct === 0) return <span className="text-[11px] text-[#8b929c]">No change</span>;
  const up = pct > 0;
  return (
...
function KpiCard({ label, value, prev, sub, icon: Icon, accent }: {
  ...
  return (
    <div className="bg-white rounded-xl px-4 py-3.5 flex items-center gap-3 border border-[var(--hairline)] card-shadow">
...
function VolumeTooltip({ active, payload, label }: any) {
  ...
  return (
    <div className="bg-white border border-[var(--hairline)] rounded-xl px-3 py-2.5 shadow-lg text-[14px]">
      <p className="font-semibold text-[#111318] mb-1">{label}</p>
      {payload.map((p: any) => (
        <div key={p.dataKey} className="flex items-center gap-2">
          <span className="w-2 h-2 rounded-full shrink-0" style={{ background: p.color }} />
          <span className="text-[#6b7280] capitalize">{p.name}:</span>
          <span className="font-semibold text-[#111318]">{p.value}</span>
        </div>
      ))}
    </div>
  );
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1">
          <h1 className="text-[18px] font-bold text-[#111318]">WhatsApp Personal - Analytics</h1>
          <p className="text-[14px] text-[#8b929c]">Message volume, contact activity and full send/receive log</p>
        </div>
        <button onClick={() => { fetchMain(period); fetchLogs(period, direction, search, logPage); }}
          className="flex items-center gap-1.5 text-[14px] font-semibold text-[#6b7280] border border-[var(--hairline)] rounded-lg px-3 py-1.5 hover:bg-[var(--accent-tint)] transition-colors">
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        {PERIODS.map((pr) => (
          <button key={pr.value} onClick={() => setPeriod(pr.value)}
            className={cn(
              'text-[14px] font-semibold px-3.5 py-1.5 rounded-full border transition-all',
              period === pr.value
                ? 'bg-[var(--brand)] text-white border-[var(--brand)] shadow-sm'
                : 'bg-white text-[#6b7280] border-[var(--hairline)] hover:border-primary/40 hover:text-[var(--brand)]',
            )}>
            {pr.label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Volume chart */}
        <div className="lg:col-span-3 bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-5">
          <div className="mb-4">
            <p className="text-[16px] font-bold text-[#111318]">Message Volume</p>
            <p className="text-[12px] text-[#8b929c] mt-0.5">Sent vs received over time</p>
          </div>
          {loading ? (
            ...
            <div className="h-[200px] flex items-center justify-center text-[15px] text-[#8b929c]">
            ...
            <ResponsiveContainer width="100%" height={210}>
              <ComposedChart data={volume.map((v) => ({ ...v, date: fmtVolumeDate(v.date) }))}
                ...
                <XAxis dataKey="date" tick={{ fontSize: 10, fill: '#8b929c' }} axisLine={false} tickLine={false} />
                <YAxis tick={{ fontSize: 10, fill: '#8b929c' }} axisLine={false} tickLine={false} />
        ...
        {/* Top contacts */}
        <div className="lg:col-span-2 bg-white rounded-2xl border border-[var(--hairline)] card-shadow p-5">
          <div className="mb-4">
            <p className="text-[16px] font-bold text-[#111318]">Top Contacts</p>
            <p className="text-[12px] text-[#8b929c] mt-0.5">Most active numbers this period</p>
          </div>
          {loading ? (
            <div className="flex flex-col gap-3">
              {[1,2,3,4,5].map((i) => (
                <div key={i} className="animate-pulse flex items-center gap-2">
                  <div className="w-7 h-7 rounded-full bg-[#eef1f4]" />
                  <div className="flex-1 space-y-1">
                    <div className="h-2.5 bg-[#eef1f4] rounded w-1/2" />
                    <div className="h-1.5 bg-[#eef1f4] rounded w-full" />
                  </div>
                  <div className="h-2.5 bg-[#eef1f4] rounded w-6" />
                </div>
              ))}
            </div>
          ) : topContacts.length === 0 ? (
            <div className="flex items-center justify-center h-[160px] text-[15px] text-[#8b929c]">
              No contacts yet
            </div>
          ) : (
            <div className="flex flex-col gap-3">
              {topContacts.map((c, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-[14px] font-semibold text-[#111318] truncate">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      <span className="text-[10px] text-[#8b929c] shrink-0">{c.sent}↑ {c.received}↓</span>
                    </div>
                  </div>
                  <span className="text-[14px] font-bold text-[#111318] shrink-0">{c.total}</span>
                </div>
              ))}
            </div>
          )}
      ...
      <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow overflow-hidden">
        {/* Log header + filters */}
        <div className="px-5 py-4 border-b border-[var(--hairline)] flex flex-col sm:flex-row sm:items-center gap-3">
          <div className="flex-1">
            <p className="text-[16px] font-bold text-[#111318]">Message Log</p>
            <p className="text-[12px] text-[#8b929c]">{logTotal.toLocaleString()} messages</p>
          </div>
          <div className="flex items-center gap-2 flex-wrap">
            {/* Direction filter */}
            <div className="flex border border-[var(--hairline)] rounded-lg overflow-hidden text-[12px] font-semibold">
              {(['all', 'sent', 'received'] as const).map((d) => (
                <button key={d} onClick={() => { setDirection(d); setLogPage(0); }}
                  className={cn(
                    'px-3 py-1.5 transition-colors capitalize',
                    direction === d ? 'bg-[var(--brand)] text-white' : 'text-[#6b7280] hover:bg-[var(--accent-tint)]',
                  )}>
                  {d}
                </button>
              ))}
            ...
            <div className="relative">
              <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#8b929c]" />
              <input
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search contact or phone…"
                className="pl-8 pr-3 py-1.5 text-[14px] border border-[var(--hairline)] rounded-lg w-48 focus:outline-none focus:border-[var(--brand)] transition-colors"
              />
            </div>
          </div>
        </div>
        ...
        {logsLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-14 text-center gap-2">
            ...
            <p className="text-[12px] text-[#8b929c]">Try a different period or filter</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-[14px]">
              <thead>
                <tr className="border-b border-[var(--hairline)] text-[11px] font-bold uppercase tracking-wider text-[#8b929c]">
              ...
              <tbody className="divide-y divide-black/5">
                {logs.map((row) => (
                  <tr key={row.id} className="hover:bg-[var(--app-bg)] transition-colors">
                    <td className="px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <div className="min-w-0">
                          <p className="font-semibold text-[#111318] truncate max-w-[140px]">
                            {row.contact_name || 'Unknown'}
                          </p>
                          {row.contact_phone && (
                            <p className="text-[11px] text-[#8b929c] truncate">{row.contact_phone}</p>
                          )}
                        </div>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-3">
                      <p className="text-[#3a3f47] line-clamp-2 max-w-[240px] leading-relaxed">
                        {row.body || <span className="text-[#9ca3af] italic">{row.type === 'image' ? <span className="inline-flex items-center gap-1"><ImageIcon className="w-3 h-3" />Image</span> : row.type === 'video' ? <span className="inline-flex items-center gap-1"><Video className="w-3 h-3" />Video</span> : row.type === 'audio' ? <span className="inline-flex items-center gap-1"><Mic className="w-3 h-3" />Audio</span> : row.type === 'document' ? <span className="inline-flex items-center gap-1"><Paperclip className="w-3 h-3" />File</span> : 'Media'}</span>}
                      </p>
                    </td>
                    <td className="px-3 py-3 text-[#6b7280] whitespace-nowrap">
                      {row.wa_account ? (
                        <span className="text-[11px] bg-[#eef1f4] text-[#6b7280] rounded px-1.5 py-0.5 font-mono">
                    ...
                    <td className="px-3 py-3">
                      {row.sender === 'agent' && row.status ? (
                        <span className={cn(
                          ...
                          row.status === 'sent'      ? 'bg-[#eef1f4] text-[#8b929c]'   :
                                                       'bg-[#eef1f4] text-[#8b929c]'
                        )}>
                          {row.status}
                        </span>
                      ) : '-'}
        ...
        {/* Pagination */}
        {totalPages > 1 && (
          <div className="px-5 py-3 border-t border-[var(--hairline)] flex items-center justify-between">
            <span className="text-[12px] text-[#8b929c]">
            ...
            <div className="flex items-center gap-1">
              <button disabled={logPage === 0} onClick={() => setLogPage((p) => p - 1)}
                className="p-1.5 rounded-lg border border-[var(--hairline)] hover:bg-[var(--accent-tint)] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <span className="text-[12px] font-semibold text-[#111318] px-2">{logPage + 1} / {totalPages}</span>
