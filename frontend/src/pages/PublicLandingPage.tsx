    ...
    case 'hero':
      return (
        <div className="px-6 sm:px-10 py-16 flex flex-col gap-5 items-center text-center" style={{
          ...
          <p className="text-[16px] sm:text-[16px] max-w-lg leading-relaxed" style={{ color: theme.muted }}>{p.subtext}</p>
          {p.btnText && (
            <a href={p.btnUrl || '#'} className="mt-2 px-8 py-3.5 rounded-xl text-[16px] font-bold shadow-lg inline-block no-underline"
    ...
    case 'button':
      return (
        <div className={`px-6 sm:px-10 py-5 flex ${alignFlex}`}>
          <a href={p.url || '#'} className="px-7 py-3 rounded-xl text-[16px] font-bold transition-all inline-block no-underline" style={
    ...
    return (
      <div className="px-6 sm:px-10 py-10" style={{ background: theme.accent }}>
        <div className="max-w-sm mx-auto text-center py-8">
          ...
          <p className="text-[16px]" style={{ color: theme.muted }}>We&apos;ll get back to you soon.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="px-6 sm:px-10 py-10" style={{ background: theme.accent }}>
      ...
      <form onSubmit={handleSubmit} className="max-w-sm mx-auto flex flex-col gap-3">
        {(p.fields ?? []).map((field: string) => (
          field === 'Message' ? (
            <textarea key={field} placeholder={field} required rows={3}
              ...
              className="w-full px-4 py-2.5 rounded-xl border text-[15px] outline-none resize-none"
        ...
        <button type="submit" disabled={submitting}
          className="w-full py-3 rounded-xl text-[16px] font-bold mt-1 shadow-sm disabled:opacity-60"
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-[16px] text-gray-500">This page doesn&apos;t exist or is no longer published.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: theme.bg }}>
      ...
      {blocks.length === 0 && (
        <div className="min-h-screen flex items-center justify-center">
          <p className="text-[16px]" style={{ color: theme.muted }}>This page has no content yet.</p>
        </div>
      )}
    </div>
  );
