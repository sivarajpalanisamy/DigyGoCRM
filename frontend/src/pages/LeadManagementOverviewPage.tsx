...
function SortableStageRow({ stage, idx, onUpdate, onRemove, onToggleWon }: {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex items-center gap-2 border border-[var(--hairline)] rounded-xl px-3 py-2 focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary/40 bg-white transition-colors">
        ...
        <button
          ...
          className={cn(
            'text-[11px] font-bold px-2 py-0.5 rounded-full border transition-colors shrink-0',
            stage.is_won
              ? 'bg-emerald-50 border-emerald-300 text-emerald-700'
              : 'bg-gray-50 border-gray-200 text-[#9ca3af] hover:border-emerald-300 hover:text-emerald-600'
          )}
        >
          Won
        </button>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-5 border-b border-[var(--hairline)] shrink-0">
          <h3 className="font-headline font-bold text-[#111318] text-[17px]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
          <div>
            <label className="text-[14px] font-semibold text-[#6b7280] mb-1.5 block">Pipeline Name <span className="text-red-400">*</span></label>
            <input
              className="w-full border border-[var(--hairline)] rounded-xl px-4 py-2.5 text-[16px] text-[#111318] outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary/40 placeholder:text-gray-400 transition"
          ...
          <div>
            ...
            <button onClick={addStage} className="mt-3 flex items-center gap-1.5 text-[15px] font-semibold text-primary hover:opacity-80 transition-opacity">
              <Plus className="w-4 h-4" /> Add Stage
            </button>
          </div>
        </div>
        <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-[var(--hairline)] shrink-0">
          <button onClick={onClose} className="px-5 py-2 rounded-xl text-[15px] font-semibold text-[#6b7280] hover:bg-gray-100 transition-colors">
            Cancel
          </button>
          <button
            onClick={handleSave}
            className="px-6 py-2 rounded-xl text-[15px] font-bold text-white transition-all hover:-translate-y-0.5"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md flex flex-col max-h-[85vh]">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-3">
          {stageStats.map((stage, idx) => (
            <div key={stage.id} className="flex items-center justify-between gap-3 py-2 border-b border-[var(--hairline)] last:border-0">
              <div className="flex items-center gap-2.5">
                <span className="text-[12px] text-[#9ca3af] w-5 shrink-0">{idx + 1}.</span>
                <span className="text-[15px] text-[#111318] font-medium">{stage.name}</span>
              </div>
              <span className="text-[15px] tabular-nums shrink-0">
                <span className={cn('font-bold', stage.count > 0 ? 'text-[#111318]' : 'text-[#c3c8cf]')}>
                  {stage.count}
                </span>
                <span className="text-[#9ca3af]">/{totalLeads} leads</span>
              </span>
            </div>
          ))}
        </div>

        <div className="flex gap-2 px-6 py-4 border-t border-[var(--hairline)] shrink-0">
          <button
            onClick={onClose}
            className="flex-1 py-2.5 rounded-xl text-[15px] font-semibold text-[#6b7280] hover:bg-gray-100 transition-colors"
          >
            Close
          </button>
          <button
            onClick={onOpen}
            className="flex-1 py-2.5 rounded-xl text-[15px] font-bold text-white transition-all hover:-translate-y-0.5"
      ...
      >
        ...
        <div className="px-5 py-4 space-y-2.5 flex-1">
          {visible.map((stage) => (
            <div key={stage.id} className="flex items-center justify-between gap-3">
              <span className="text-[15px] text-[#3a3f47] font-medium truncate">{stage.name}</span>
              <span className="text-[15px] tabular-nums shrink-0">
                <span className={cn('font-bold', stage.count > 0 ? 'text-[#111318]' : 'text-[#c3c8cf]')}>
                  {stage.count}
                </span>
                <span className="text-[#9ca3af] font-normal">/{totalLeads} leads</span>
              </span>
            </div>
          ))}
        ...
        <div className="px-4 pb-4 flex items-center gap-2" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="flex-1 py-2 rounded-xl text-[14px] font-semibold text-[#6b7280] bg-[var(--surface-2)] hover:bg-[var(--accent-tint)] active:scale-[0.98] transition"
          >
            All Stages
          </button>
          <button
            onClick={onView}
            className="flex-1 py-2 rounded-xl text-[14px] font-bold text-primary bg-orange-50 hover:bg-orange-100 active:scale-[0.98] transition"
          >
            Go to Board
          </button>
        </div>
  ...
  return (
    <div className="space-y-5 animate-fade-in">
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <select
          className="pl-3 pr-8 py-2.5 text-[15px] bg-white border border-[var(--hairline)] rounded-xl outline-none focus:border-primary/40 text-[#111318] appearance-none cursor-pointer"
          style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}
        ...
        {canManage && (
          <button
            ...
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl text-[15px] font-bold text-white transition-all hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98] shrink-0"
      ...
      {filtered.length > 0 ? (
        ...
        <div className="bg-white rounded-2xl border border-[var(--hairline)] card-shadow py-20 flex flex-col items-center gap-3 text-center">
          ...
          {canManage ? (
            <>
              <p className="font-semibold text-[#111318] text-[16px]">No pipelines yet</p>
              <p className="text-[15px] text-[#6b7280] max-w-xs">Create your first pipeline to start organising leads by stage</p>
              <button
                onClick={() => setShowCreate(true)}
                className="mt-2 flex items-center gap-2 px-5 py-2.5 rounded-xl text-[15px] font-bold text-white"
            ...
            <>
              <p className="font-semibold text-[#111318] text-[16px]">No pipelines assigned to you</p>
              <p className="text-[15px] text-[#6b7280] max-w-xs">You can only view pipelines where leads are assigned to you</p>
            </>
          )}
        </div>
      )}
      ...
      {deletePipelineTarget && (
        <ConfirmDeleteModal
          ...
          message={<>Delete <span className="font-semibold text-[#111318]">"{deletePipelineTarget.name}"</span>? All leads in this pipeline will lose their stage assignment. This cannot be undone.</>}
